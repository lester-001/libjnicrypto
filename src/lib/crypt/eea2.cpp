...
void Encrypt_c(const uint8_t *key, uint32_t count, int bearer, int direction, uint8_t *buffer, size_t length)
{
    uint8_t iv[16] = {0};
    ComputeIv(iv, count, bearer, direction);
    Cipher(key, iv, buffer, length);
}
...
void Decrypt_c(const uint8_t *key, uint32_t count, int bearer, int direction, uint8_t *buffer, size_t length)
{
    uint8_t iv[16] = {0};
    ComputeIv(iv, count, bearer, direction);
    Cipher(key, iv, buffer, length);
